...
def create_loss_analysis_plots(
    df: pd.DataFrame, model_size: float, training_type: str, metrics: List[str]
    ...
    val_loss_fig = make_subplots(
        rows=len(metrics),
        cols=1,
        subplot_titles=[f"{m.replace('_mean', '').replace('_', ' ').title()} vs Validation Loss" for m in metrics],
    ...
    train_loss_fig = make_subplots(
        rows=len(metrics),
        cols=1,
        subplot_titles=[f"{m.replace('_mean', '').replace('_', ' ').title()} vs Training Loss" for m in metrics],
    ...
    for i, metric in enumerate(metrics, 1):
        ...
        hover_text = [
            ...
            for model, size, tokens, val_loss, value in zip(
        ...
        if len(filtered_df) > 2:
            ...
            p = np.poly1d(z)
            x_range = np.linspace(filtered_df["best_val_loss"].min(), filtered_df["best_val_loss"].max(), 100)
            val_loss_fig.add_trace(
                go.Scatter(x=x_range, y=p(x_range), mode="lines", line=dict(dash="dash", color="red"), showlegend=False),
                row=i,
                col=1,
            )
        ...
        hover_text = [
            ...
            f"{metric.replace('_mean', '')}: {value:.3f}"
            for model, size, tokens, train_loss, value in zip(
                filtered_df["model_name"],
                filtered_df["model_size_M"],
                filtered_df["total_tokens_M"],
        ...
        if len(filtered_df) > 2:
            ...
            p = np.poly1d(z)
            x_range = np.linspace(filtered_df["train_loss"].min(), filtered_df["train_loss"].max(), 100)
            train_loss_fig.add_trace(
                go.Scatter(x=x_range, y=p(x_range), mode="lines", line=dict(dash="dash", color="red"), showlegend=False),
                row=i,
                col=1,
    ...
    val_loss_fig.update_layout(
        height=300 * len(metrics), title=f"Metrics vs Validation Loss for {model_size}M Model ({training_type})", showlegend=False
    )

    train_loss_fig.update_layout(
        height=300 * len(metrics), title=f"Metrics vs Training Loss for {model_size}M Model ({training_type})", showlegend=False
    )

    return val_loss_fig, train_loss_fig


def create_loss_progression_plot(df: pd.DataFrame, model_size: float, training_type: str) -> go.Figure:
    ...
    # Add validation loss
    hover_text = [
        f"Model: {model}<br>" f"Tokens: {tokens:.1f}M<br>" f"Val Loss: {val_loss:.3f}"
        for model, tokens, val_loss in zip(filtered_df["model_name"], filtered_df["total_tokens_M"], filtered_df["best_val_loss"])
    ]

    fig.add_trace(
        go.Scatter(
            x=filtered_df["total_tokens_M"],
    ...
    # Add training loss
    hover_text = [
        f"Model: {model}<br>" f"Tokens: {tokens:.1f}M<br>" f"Train Loss: {train_loss:.3f}"
        for model, tokens, train_loss in zip(filtered_df["model_name"], filtered_df["total_tokens_M"], filtered_df["train_loss"])
    ]
...
def load_and_process_data(file_path: str) -> pd.DataFrame:
    ...
    df["token_range"] = pd.qcut(df["total_tokens_M"], q=5, labels=["0-20%", "20-40%", "40-60%", "60-80%", "80-100%"])
...
def plot_metrics_by_size(df: pd.DataFrame, token_range: str, training_type: str, metrics: List[str]) -> go.Figure:
    ...
    fig = make_subplots(
        rows=len(metrics),
        cols=1,
        subplot_titles=[f"{m.replace('_mean', '').replace('_', ' ').title()} vs Model Size" for m in metrics],
    )

    for i, metric in enumerate(metrics, 1):
        # Add scatter plot
        hover_text = [
            f"Model: {model}<br>" f"Size: {size}M<br>" f"Tokens: {tokens:.1f}M<br>" f"{metric.replace('_mean', '')}: {value:.3f}"
            for model, size, tokens, value in zip(
                filtered_df["model_name"], filtered_df["model_size_M"], filtered_df["total_tokens_M"], filtered_df[metric]
            )
        ]

        fig.add_trace(
            go.Scatter(
                x=filtered_df["model_size_M"],
                y=filtered_df[metric],
                mode="markers+text",
...
def create_token_progression_plot(df: pd.DataFrame, model_size: float, training_type: str, metrics: List[str]) -> go.Figure:
    ...
    fig = make_subplots(
        rows=len(metrics),
        cols=1,
        subplot_titles=[f"{m.replace('_mean', '').replace('_', ' ').title()} vs Training Tokens" for m in metrics],
    )
    ...
    fig.update_layout(
        height=300 * len(metrics), title=f"Training Progression for {model_size}M Model ({training_type})", showlegend=False
    )
...
def main():
    ...
    if uploaded_file is not None:
        ...
        training_type = st.sidebar.selectbox(
            "Select Training Type", options=sorted(df["training_type"].unique()), help="Filter models by training approach"
        )
        ...
        metric_names = [col for col in df.columns if "mean" in col]
        metrics = st.sidebar.multiselect(
            "Select Metrics to Compare", options=metric_names, default=metric_names, help="Choose metrics to display in plots"
        )

        # Display basic statistics for filtered data
        filtered_df = df[(df["token_range"] == token_range) & (df["training_type"] == training_type)]
        ...
        with col2:
            st.metric("Token Range (M)", f"{filtered_df['total_tokens_M'].min():.1f} - {filtered_df['total_tokens_M'].max():.1f}")
        with col3:
            st.metric("Size Range (M)", f"{filtered_df['model_size_M'].min():.0f} - {filtered_df['model_size_M'].max():.0f}")
        with col4:
            st.metric("Avg Val Loss", f"{filtered_df['best_val_loss'].mean():.3f}")
        ...
        model_size = st.selectbox("Select Model Size (M)", options=sorted(filtered_df["model_size_M"].unique()))
        ...
        loss_stats = filtered_df.agg(
            {"best_val_loss": ["mean", "std", "min", "max"], "train_loss": ["mean", "std", "min", "max"]}
        ).round(4)
        ...
        st.dataframe(
            filtered_df[["model_name", "model_size_M", "total_tokens_M", "best_val_loss"] + metrics].sort_values(
                ["model_size_M", "total_tokens_M"]
            )
        )
